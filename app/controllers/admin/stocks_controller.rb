class Admin::StocksController < AdminController
  ...
  def index
    @admin_stocks = @admin_product.stocks.all
  end
  ...
  def update
    respond_to do |format|
      if @admin_stock.update(admin_stock_params)
        format.html { redirect_to admin_product_stock_url(@admin_product, @admin_stock), notice: "Stock was successfully updated." }
  ...
  private
    ...
    def admin_stock_params
      params[:stock][:product_id] = @admin_product.id
