class AdminController < ApplicationController
  ...
  def index
    # TO DO
    # Improve fetchign of data
    @orders = Order.fulffiled.order(created_at: :asc).take(5)
    @quick_stats = {
      ...
      per_sale: OrderProduct.joins(:order).where(created_at: Time.now.midnight..Time.now).average(:quantity),
    }
